# JPP Website - Automated Setup Script for Windows
# Run this in PowerShell from your project directory

Write-Host "🎭 JPP Website - Automated Improvement Setup" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""

# Check if we're in the right directory
if (-not (Test-Path "index.html")) {
    Write-Host "⚠️  Warning: index.html not found in current directory" -ForegroundColor Yellow
    Write-Host "Current directory: $(Get-Location)"
    Write-Host "Please run this script from your project root directory"
    exit 1
}

Write-Host "📁 Found project files" -ForegroundColor Blue
Write-Host ""
...
$cssContent | Out-File -FilePath "jpp-improvements.css" -Encoding UTF8
...
$jsContent | Out-File -FilePath "jpp-improvements.js" -Encoding UTF8
...
if ($html -notmatch "jpp-improvements.css") {
    $html = $html -replace '</head>', "  <link rel=`"stylesheet`" href=`"jpp-improvements.css`">`r`n</head>"
    Write-Host "✅ Added CSS link" -ForegroundColor Green
} else {
    Write-Host "ℹ️  CSS link already exists" -ForegroundColor Yellow
}

# Add JS link if not present
if ($html -notmatch "jpp-improvements.js") {
    $html = $html -replace '</body>', "  <script src=`"jpp-improvements.js`"></script>`r`n</body>"
    Write-Host "✅ Added JS link" -ForegroundColor Green
} else {
    Write-Host "ℹ️  JS link already exists" -ForegroundColor Yellow
}

# Add Inter font if not present
if ($html -notmatch "fonts.googleapis.com.*Inter") {
    $fontLinks = @"
...
"@
    ...
    Write-Host "ℹ️  Inter font already exists" -ForegroundColor Yellow
}
...
Write-Host "2. Check http://localhost:8000" -ForegroundColor Cyan
Write-Host "3. Commit changes:" -ForegroundColor Cyan
Write-Host "   git add ."
Write-Host "   git commit -m 'feat: implement comprehensive site improvements'"
Write-Host "   git push origin main"
Write-Host ""
Write-Host "Created files:" -ForegroundColor Green
Write-Host "  ✅ jpp-improvements.css"
Write-Host "  ✅ jpp-improvements.js"
Write-Host "  ✅ index.html.bak (backup)"
Write-Host ""
Write-Host "🎉 Your site is now improved!" -ForegroundColor Magenta
Write-Host ""
